 ...
 * @author djemili
 */
public class Category extends Events {
    ...
    public void shuffleDeck(int level) {
        if (level >= 0 && level < m_decks.size()) {
            List<Card> deck = m_decks.get(level);
            Collections.shuffle(deck);
        } else {
            throw new IllegalArgumentException("Invalid deck level: " + level);
        }
    }
}
